...
import { normalizeMedia, getMediaCategory } from '../../lib/media.js';

let cleanup = null;
let pendingMedia = null; // { bytes, contentType, originalName }
...
export function mount(container, client, router) {
  ...
  mediaInput.addEventListener('change', async () => {
    ...
    if (file) {
      try {
        // Normalize media (converts HEIC to JPEG, etc.)
        const normalized = await normalizeMedia(file);
        pendingMedia = normalized;

        const category = getMediaCategory(normalized.contentType);
        const icon = category === 'video' ? '🎬' : category === 'audio' ? '🎵' : '📎';
        mediaPreview.textContent = `${icon} ${file.name}`;
        mediaPreview.classList.remove('hidden');
      } catch (err) {
        console.error('[CreateStory] Failed to process media:', err);
        mediaPreview.textContent = `❌ Failed to process ${file.name}`;
        mediaPreview.classList.remove('hidden');
      }
    }
  });
